...
def generate_ground_truth(ground_truth_path, vocabulary, max_arity, max_seq_length, dataset):
    """
    ...
    all_examples, _ = read_examples(ground_truth_path, max_arity, dataset)
...
def batch_evaluation(global_idx, batch_results, all_features, gt_dict, is_temp, r_num, e_num):
    ret_ranks = {
        'entity': [],
        'relation': [],
        '2-r': [],
    ...
    for i, result in enumerate(batch_results):
        ...
        if(is_temp and pos in [0,2]):
            result[2+e_num+r_num:] = -np.Inf #remove time in the candidate
        rank = ((result >= result[target]).sum() + (result > result[target]).sum()+1)//2
        assert rank >= 1
